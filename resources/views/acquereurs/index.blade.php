...
@section('content')
    <div class="block block-rounded">
        <div class="block-header">
            ...
            <a href="{{ route('acquereur.create') }}" class="btn btn-outline-primary">
        ...
        <div class="block-content block-content-full">
            <table class="table table-bordered table-striped table-vcenter js-dataTable-full-pagination">
                ...
                <tbody>
                    @forelse ($acquereurs as $item)
                    <tr>
                        <td class="text-center font-size-sm"> {{$item->id}}</td>
                        <td class="text-center font-size-sm text-uppercase"> {{$item->nom}} </td>
                        <td class="d-none d-sm-table-cell text-center text-capitalize"> {{$item->prenom}}</td>
                        <td class="text-center d-none d-sm-table-cell font-size-sm"> {{$item->contact}}</td>
                        <td class="text-center d-none d-sm-table-cell font-size-sm">{{$item->email}} </td>
                        ...
                        <td class="text-center"> 
                            ...
                            <form action="{{route('acquereur.edit',$item)}}" method="GET">
                                <button type="submit" class="btn btn-sm btn-warning">
                                    <span><i class="far fa-edit fa-lg"></i></span>
                                </button> 
                            </form>                                      
                            <form action=" {{route('acquereur.delete',$item)}}" method="POST">
                                @csrf
                                @method("delete")
                    ...
                    @empty
                        <div class="my-auto">
                            <h3 class="text-center">Aucun acquereur.</h3>
                        </div>
                    @endforelse
                </tbody>
            </table>
